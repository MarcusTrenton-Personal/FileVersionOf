...
msbuild "FileVersionOf\FileVersionOf.csproj" -target:publish -property:PublishProfile=\FileVersionOf\Properties\PublishProfiles\Local.pubxml -verbosity:quiet
...
for /f %%i in ('FileVersionOf\bin\Release\net7.0\FileVersionOf.exe FileVersionOf\bin\Release\net7.0\FileVersionOf.exe') do set VERSION=%%i
...
dotnet nuget push "FileVersionOf\bin\Release\FileVersionOf.%VERSION%.nupkg" --api-key %NugetGitToken% --source "github"
if NOT %errorlevel% == 0 exit /B %errorlevel%
